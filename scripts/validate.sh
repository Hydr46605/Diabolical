...
[[ -f docs/guides/verification.md ]] || fail "missing verification guide"
[[ -f docs/guides/manual-playtest.md ]] || fail "missing manual playtest guide"
...
for profile_file in apply_current sync_scores calm default hard apply/calm apply/default apply/hard; do
  [[ -f "data/diabolical/function/config/profile/${profile_file}.mcfunction" ]] || fail "missing balance profile function: ${profile_file}"
done
...
python3 - <<'PY' || fail "structure NBT files are invalid"
import gzip
...
class NbtReader:
    ...
    def read(self, size):
        ...
        if end > len(self.data):
            raise ValueError(f"{self.path}: truncated NBT payload at byte {self.offset}")
    ...
    def skip_payload(self, tag_type):
        if tag_type == TAG_BYTE:
            ...
            length = self.int()
            self.check_length(length, "byte array")
            self.read(length)
            ...
            length = self.int()
            self.check_length(length, "list")
            if child_type == 0 and length != 0:
                raise ValueError(f"{self.path}: TAG_End list has nonzero length")
            ...
            length = self.int()
            self.check_length(length, "int array")
            self.read(length * 4)
        elif tag_type == TAG_LONG_ARRAY:
            length = self.int()
            self.check_length(length, "long array")
            self.read(length * 8)
        else:
            raise ValueError(f"{self.path}: unknown NBT tag type {tag_type}")

    def check_length(self, length, label):
        if length < 0:
            raise ValueError(f"{self.path}: negative {label} length")

found = set()
for path in Path("data/diabolical/structure").glob("*.nbt"):
    raw = gzip.decompress(path.read_bytes())
    reader = NbtReader(path, raw)
    root_type = reader.ubyte()
    if root_type != TAG_COMPOUND:
        raise SystemExit(f"{path}: root tag is not a named compound")

    reader.skip_name()
    reader.skip_payload(root_type)
    if reader.offset != len(raw):
        raise SystemExit(f"{path}: trailing bytes after root compound")
