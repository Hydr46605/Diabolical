...
```sh
PAPER_SMOKE_TIMEOUT=420 scripts/paper_smoke.sh 0.4.2-rc1
```

Local Mojang jar:

```sh
scripts/paper_smoke.sh 0.4.2-rc1 --mojang-jar path/to/mojang_1.21.11.jar
```
...
1. Put `dist/Diabolical-0.4.2-rc1.zip` in a fresh world's `datapacks/` folder.
2. Run `/reload`.
3. Run `function diabolical:debug/status`.
4. Run `function diabolical:config/status`.
5. Confirm the version is `0.4.2-rc1`, runtime phase is `24`, and the balance
   profile is `default`.
...
4. Right-click the Ledger and open Status, Contracts, Payments, Relics, Court,
   and Help.
5. Sign one minor contract and confirm Payments shows the active pact.
6. Pay or revoke the pact and confirm debt/sin changes are readable.
...
1. Install `dist/Diabolical-optional-resources-0.4.2-rc1.zip` as a resource pack.
2. Confirm the Primer, Ledger, relics, compass items, and court items render with
   custom 16x16 textures.
3. Remove the resource pack and confirm the datapack still plays normally with
   vanilla item visuals.
